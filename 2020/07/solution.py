...
def count_bags(bag_type, bag_ruleset):
    count = set()
    for k, v in bag_ruleset.items():
        if bag_type in v:
            count.add(k)
            count = count.union(count_bags(k, bag_ruleset))
    return count


def part1(rules):
    bag_pattern = re.compile("(\w+\s\w+)\sbag[s]+")
    bags = {}
    for rule in rules:
        r = bag_pattern.findall(rule)
        bags.update({r[0]: set(r[1:])})

    bag_total = count_bags("shiny gold", bags)
    print("Part 1: {} bags".format(len(bag_total)))
