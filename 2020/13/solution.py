...
def readInput():
    with open('input.txt', 'r') as f:
        schedule = [i for i in f.read().strip().split()]
    time = int(schedule[0])
    bus_ids = schedule[1].split(",")
    return time, bus_ids


def part1(time, bus_ids):
    bus_list = [int(i) for i in list(filter(lambda x: x != "x", bus_ids))]
    bus_id = -1
    wait = -1
    for id in bus_list:
        temp_wait = id - (time % id)
        if wait == -1:
            wait = temp_wait
            bus_id = id
        elif temp_wait < wait:
    ...
    print("Part 1: {}".format(bus_id * wait))


def main():
    time, bus_ids = readInput()
    part1(time, bus_ids)
