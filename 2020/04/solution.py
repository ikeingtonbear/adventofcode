...
def readInput():
    ...
    with open('./input.txt', 'r') as f:
        passports = [" ".join(p.strip().split("\n")) for p in
                     f.read().strip().split("\n\n")]
    return passports


def validatePassport(passport):
    completely_valid = {"byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid", "cid"}
    fields = {i.split(":")[0] for i in passport.split()}
    diff = completely_valid.difference(fields)
    if diff == {"cid"} or not diff:
        return True
    return False
...
def part1(passports):
    ...
    for passport in passports:
        if validatePassport(passport):
            valid_passports += 1
    print("Part 1: {} valid passports".format(valid_passports))
    ...
    for passport in passports:
        if validatePassport(passport):
            if pid_re.match(passport):
                if ecl_re.match(passport):
                    if hcl_re.match(passport):
                        if byr_re.match(passport):
                            byr = int(byr_re.match(passport).group(1))
                            if 1920 <= byr and byr <= 2002:
                                if iyr_re.match(passport):
                                    iyr = int(iyr_re.match(passport).group(1))
                                    if 2010 <= iyr and iyr <= 2020:
                                        if eyr_re.match(passport):
                                            eyr = int(eyr_re.match(passport).group(1))
                                            if 2020 <= eyr and eyr <= 2030:
                                                if hgt_re.match(passport):
                                                    val = int(hgt_re.match(passport).group(1))
                                                    measure = hgt_re.match(passport).group(2)
                                                    if "cm" in measure and 150 <= val and val <= 193:
                                                        valid_passports += 1
                                                    if "in" in measure and 59 <= val and val <= 76:
                                                        valid_passports += 1

    print("Part 2: {} valid passports".format(valid_passports))
