...
def readInput2():
    answers = []
    with open('./input.txt', 'r') as f:
        answers = [p.split() for p in
                   f.read().strip().split("\n\n")]
    return answers


def part1(answers):
    total = reduce(lambda x, y: x+y, map(lambda x: len(set(x)), answers))
    print("Part 1: {} answers".format(total))


def part2(answers):
    total = reduce(lambda x, y: x+y,
                   map(lambda a: len(reduce(lambda x, y: x.intersection(y),
                                            map(lambda x: set(x),
                                                a))),
                       answers))
    print("Part 2: {} answers".format(total))
