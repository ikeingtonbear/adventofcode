...
def part1(passwords):
    compliant = 0
    pattern = re.compile('^(\d+)-(\d+)\s(.):\s(.+)$')
    for p in passwords:
        min, max, char, str = pattern.match(p).group(1, 2, 3, 4)
        count = str.count(char)
        if int(min) <= count and count <= int(max):
            compliant += 1
    print("Part 1: {} compliant passwords".format(compliant))
...
def part2(passwords):
    ...
    for p in passwords:
        ...
        min = int(min) - 1
        max = int(max) - 1
        if (str[min] == char or str[max] == char) and not \
                (str[min] == char and str[max] == char):
            compliant += 1
    print("Part 2: {} compliant passwords".format(compliant))
