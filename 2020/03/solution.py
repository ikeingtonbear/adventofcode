...
def readInput():
    with open('./input.txt', 'r') as f:
        pattern = [i.strip() for i in f.readlines()]
        return pattern


def toboggan_run(pattern, run, rise):
    height = len(pattern)
    width = len(pattern[0])
    curr_h = 0
    curr_w = 0
    ...
    for i in range(rise, height, rise):
        curr_w = (curr_w + run) % width
        if pattern[i][curr_w] == "#":
            num_trees += 1
...
def part1(pattern):
    num_trees = toboggan_run(pattern, 3, 1)
    print("Part 1: {} trees".format(num_trees))


def part2(pattern):
    num_trees = toboggan_run(pattern, 1, 1) * toboggan_run(pattern, 3, 1) *\
                toboggan_run(pattern, 5, 1) * toboggan_run(pattern, 7, 1) *\
                toboggan_run(pattern, 1, 2)
    print("Part 2: {} trees".format(num_trees))
